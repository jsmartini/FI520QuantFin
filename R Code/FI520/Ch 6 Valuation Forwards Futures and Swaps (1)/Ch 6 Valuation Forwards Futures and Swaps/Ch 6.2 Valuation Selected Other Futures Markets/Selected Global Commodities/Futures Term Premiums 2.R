...
legend("bottom", legtxt, horiz = TRUE, cex = 0.75, lwd = c(1,1), 
  lty = c(1,1,1), col = c("black","black"), bty = "n", pch = c(1, 2),   
  inset = c(0, 0))
par(defaultpar) # Reset to default parameters
# #
# # Futures Basis Plots
...
# par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0),new = TRUE)
# plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
# # legend("bottom", legtxt, horiz = TRUE, cex = 0.75, lwd = c(1), 
# #        lty = c(1), col = c("black"), bty = "n", pch = c(1),   
# #        inset = c(0, 0))
# par(defaultpar) # Reset to default parameters
#
# Term Premium Plots
...
y2 <- FP$TP3/(FP$Mat3 - FP$Mat2)
if(FixYRangeTP){
  MaxYValue = MaxYValueTP
  MinYValue = MinYValueTP
} else {
  MaxYValue = max(y1, y2, na.rm = TRUE)
  MinYValue = min(y1, y2, na.rm = TRUE)
}
...
MaxXValue = max(xI); MinXValue = min(xI)
xlim1 = c(1:2); xlim1[1] = MinXValue; xlim1[2] = MaxXValue
legtxt = c("3M Libor/Shibor", "Term Premium 3 (Annualized)")
mTitle = paste0(Title1," Marginal Term Premium and 3M Libor")
xTitle = "Calendar Date"
...
plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
legend("bottom", legtxt, horiz = TRUE, cex = 0.75, lwd = c(1, 1), 
  lty = c(1, 1), col = c("black","black"), bty = "n", 
  pch = c(1, 2), inset = c(0, 0))
par(defaultpar) # Reset to default parameters
