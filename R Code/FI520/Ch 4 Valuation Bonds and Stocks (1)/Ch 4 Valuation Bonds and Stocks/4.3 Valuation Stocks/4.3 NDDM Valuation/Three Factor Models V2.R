# Three Factor Models V2.R
#  Version 2: forward LSC model
# Three factor, lnPVD, with intercept
#
LSC <- lm(formula = Div$lnPVD ~ Factors2[1,] + Factors2[2,] )
...
if(PlotlnPVD){
  ...
  plot(Div$MaturityTime, Div$lnPVD, type="l", 
    main="LSC Model V2-Three Factors (Gray-Fitted, Black-lnPVD)",
...
#
LSC <- lm(formula = Div$PVD ~ Factors2[1,] + Factors2[2,])
Betas <- LSC$coefficients # Make clear grabbing beta coefficients
Level <- Betas[1]
Slope <- Betas[2]
Curvature1 <- Betas[3]
...
if(PlotPVD){
  ...
  plot(Div$MaturityTime, Div$PVD, type="l", 
    main="LSC Model V2-Three Factors (Gray-Fitted, Black-PVD)", 
    xlab="Maturity", ylab="PVD", col="black", xlim = xlim1, ylim = ylim1,
    pch = 2, cex = 0.5)
