# Six Factor Models.R
#
# Six factor, lnPVD, with intercept
#
...
PredictedlnPVD <- Level + Slope*Factors[1,] + Curvature1*Factors[2,] +
  Curvature2*Factors[3,] + Curvature3*Factors[4,] + Curvature4*Factors[5,]
...
for (j in 1:NumberOfMaturities){
  FittedlnPVD[i,j] = Level + Slope * Factors[1,j] + Curvature1*Factors[2,j] +
    Curvature2*Factors[3,j] + Curvature3*Factors[4,j] + Curvature4*Factors[5,j]
}
# Plots
if(PlotlnPVD){
  MinMaturity = 0; MaxMaturity = max(Div$MaturityTime)
  xlim1 = c(1:2); xlim1[1] = MinMaturity; xlim1[2] = MaxMaturity
  MinlnPVD = min(Div$lnPVD, FittedlnPVD); MaxlnPVD = max(Div$lnPVD, FittedlnPVD)
  ylim1 = c(1:2); ylim1[1] = MinlnPVD; ylim1[2] = MaxlnPVD
  plot(Div$MaturityTime, Div$lnPVD, type="l", 
    main="LSC Model-Six Factors (Gray-Fitted, Black-lnPVD)",
    xlab="Maturity", ylab="PVD", col="black", xlim = xlim1, ylim = ylim1,
    pch = 2, cex = 0.5)
  lines(Div$MaturityTime, FittedlnPVD[1,], type="l", col="gray", xlim = xlim1,
...
if(PlotPVD){
  ...
  plot(Div$MaturityTime, Div$PVD, type="l", 
    main="LSC Model-Six Factors (Gray-Fitted, Black-PVD)", 
    xlab="Maturity", ylab="PVD", col="black", xlim = xlim1, ylim = ylim1,
    pch = 2, cex = 0.5)
  lines(Div$MaturityTime, FittedPVD[1,], type="l", col="gray", xlim = xlim1, 
    ylim = ylim1, pch = 2, cex = 0.5)
}
